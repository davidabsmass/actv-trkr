...
export default function NotificationsHub() {
  ...
  const { data: inbox, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: orgSites } = await supabase.from("sites").select("id").eq("org_id", orgId);
  ...
  const getSubEnabled = (siteId: string, alertType: string, channel: string) => {
    ...
    return sub?.is_enabled ?? true;
  };
  const unreadCount = inbox?.filter(n => !n.is_read).length || 0;

  // Auto-mark all as read when this hub is viewed
  useEffect(() => {
  ...
  return (
    <Tabs defaultValue="inbox" className="space-y-4">
      <TabsList>
        ...
        <TabsTrigger value="leads">Lead Alerts</TabsTrigger>
