...
function buildFiles(apiKey: string): Record<string, string> {
  const bakedKeyDefine = apiKey ? `\ndefine('AT_BAKED_API_KEY','${apiKey}');` : '';
  const activationBake = apiKey ? `\n\$opts=get_option('at_options',array());\n\$opts['api_key']=AT_BAKED_API_KEY;\n\$opts['endpoint_url']=AT_DEFAULT_ENDPOINT;\nif(!isset(\$opts['enable_tracking']))\$opts['enable_tracking']='1';\nif(!isset(\$opts['enable_forms']))\$opts['enable_forms']='1';\nif(!isset(\$opts['enable_heartbeat']))\$opts['enable_heartbeat']='1';\nupdate_option('at_options',\$opts);` : '';
  const preConfiguredNotice = apiKey
    ? `<?php if(defined('AT_BAKED_API_KEY')&&!empty(AT_BAKED_API_KEY)):?><div class="notice notice-success"><p><strong>✅ Pre-configured!</strong> This plugin was downloaded with your API key already set. Tracking is active.</p></div><?php endif;?>`
    : '';
...
define('AT_DEFAULT_ENDPOINT','${ENDPOINT_BASE}');${bakedKeyDefine}
...
function at_activate(){
  ...
  if(!wp_next_scheduled('at_form_probe_cron')){wp_schedule_event(time(),'hourly','at_form_probe_cron');}${activationBake}
}
...
public static function defaults(){return array('api_key'=>defined('AT_BAKED_API_KEY')?AT_BAKED_API_KEY:'','endpoint_url'=>defined('AT_DEFAULT_ENDPOINT')?AT_DEFAULT_ENDPOINT:'${ENDPOINT_BASE}','enable_tracking'=>'1','enable_forms'=>'1','enable_heartbeat'=>'1');}
...
if(!defined('ABSPATH'))exit;
class AT_Forms{
public static function init(){\$opts=AT_Settings::get();
if(is_admin()&&!empty(\$opts['api_key'])){add_action('admin_init',array(__CLASS__,'maybe_auto_sync'));}
if(\$opts['enable_forms']!=='1'||empty(\$opts['api_key']))return;
...
if(empty(\$discovered)){return array('synced'=>0,'discovered'=>0);}
...
private static function extract_links(\$html,\$page_url){\$host=wp_parse_url(home_url(),PHP_URL_HOST);preg_match_all('/href=["\\']([^"\\' ]+)["\\']/i',\$html,\$matches);\$links=array();foreach(\$matches[1] as \$href){if(strpos(\$href,'#')===0||strpos(\$href,'mailto:')===0||strpos(\$href,'tel:')===0)continue;if(strpos(\$href,'javascript:')===0)continue;if(strpos(\$href,'/')===0){\$href=home_url(\$href);}\$link_host=wp_parse_url(\$href,PHP_URL_HOST);if(\$link_host&&\$link_host!==\$host)continue;\$links[]=\$href;}return array_unique(array_slice(\$links,0,50));}
}`,
...
${apiKey ? 'This plugin was pre-configured with your API key — just install, activate, and tracking starts automatically.' : 'Paste your API key in Settings → ACTV TRKR and tracking starts automatically.'}
...
${apiKey ? "3. That's it! Tracking starts automatically. Visit Settings → ACTV TRKR to verify." : "3. Go to Settings → ACTV TRKR and paste your API key\n4. That's it! Tracking starts automatically."}
