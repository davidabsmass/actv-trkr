...
import {
  Archive, Download, ArrowLeft, CalendarIcon, FileText, Users,
...
export default function Archives() {
  ...
  // Fetch running archive export jobs
  ...
  // Group manifests by month
  const monthGroups: MonthGroup[] = (() => {
    if (!manifests || manifests.length === 0) return [];
    const groups: Record<string, MonthGroup> = {};

    for (const m of manifests) {
      const monthKey = m.start_date.substring(0, 7); // "2026-01"
      if (!groups[monthKey]) {
  ...
  // Export a category for a given month
  const exportArchive = useMutation({
    mutationFn: async ({ month, tableName }: { month: string; tableName: string }) => {
      ...
      const { error: fnError } = await supabase.functions.invoke("process-archive-export", {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-6">
        Access older detailed records for {orgName}
      </p>
                  ...
                  <CollapsibleContent>
                    <div className="border-t border-border divide-y divide-border">
                      {Object.entries(group.categories).map(([catKey, catData]) => {
                        const catConfig = ARCHIVE_CATEGORIES[catKey] || {
                          label: catKey, description: "", icon: FileText,
                        };
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <p className="text-sm font-medium text-foreground">
                                  {group.label} — {catConfig.label}
                                </p>
                                <p className="text-xs text-muted-foreground">
                                  {catData.totalRecords.toLocaleString()} records · {formatBytes(catData.totalSize)}
                                </p>
                              </div>
                            </div>
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => exportArchive.mutate({ month: group.month, tableName: catKey })}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground">
                      ...
                      {job.row_count != null && (
                        <span className="text-muted-foreground font-normal"> · {job.row_count} rows</span>
                      )}
                    </p>
                    <p className="text-xs text-muted-foreground">
