...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Note: pre-login "new device requested a sign-in code" alert was removed
    // because it fired on every code request (browser updates change UA strings,
    // trust is only granted after successful sign-in, etc.). The post-sign-in
    // `new_device_login` alert still covers the real security case.
