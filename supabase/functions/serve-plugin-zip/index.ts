...
public static function handle_avada($data,$form_post_id){$fields=array();if(is_array($data)){foreach($data as $k=>$v){$fields[]=array('name'=>$k,'label'=>$k,'type'=>'text','value'=>is_array($v)?implode(', ',$v):$v);}}$title='Avada Form';$p=get_post($form_post_id);if($p){$title=$p->post_title?:$title;}$entry_id=self::get_avada_db_entry_id($form_post_id);self::send(array('provider'=>'avada','entry'=>array('form_id'=>$form_post_id,'form_title'=>$title,'entry_id'=>$entry_id,'source_url'=>wp_get_referer()?:home_url(),'submitted_at'=>current_time('c')),'context'=>self::get_tracking_context(),'fields'=>$fields));}
private static function get_avada_db_entry_id($form_post_id){global $wpdb;$table=$wpdb->prefix.'fusion_form_submissions';if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$table))===$table){$row=$wpdb->get_row($wpdb->prepare("SELECT id FROM {$table} WHERE form_id=%d ORDER BY id DESC LIMIT 1",intval($form_post_id)));if($row){return 'avada_db_'.$row->id;}}return 'avada_'.time().'_'.wp_rand();}
