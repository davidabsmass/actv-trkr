...
private static function get_active_entry_ids($provider,$form_id,$page_url=null){global $wpdb;switch($provider){case 'gravity_forms':if(!class_exists('GFAPI'))return null;$entries=\\GFAPI::get_entries($form_id,array('status'=>'active'),null,array('offset'=>0,'page_size'=>5000));if(!is_array($entries))return array();return array_map(function($e){return (string)($e['id']??'');},$entries);case 'wpforms':if(!function_exists('wpforms')||!isset(wpforms()->entry))return null;$entries=wpforms()->entry->get_entries(array('form_id'=>$form_id));if(!is_array($entries))return array();return array_map(function($e){return (string)$e->entry_id;},$entries);case 'avada':$table=$wpdb->prefix.'fusion_form_submissions';if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$table))!==$table)return null;static $avada_global_rows=null;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,date_time FROM {$table} WHERE form_id=%d ORDER BY id DESC LIMIT 5000",intval($form_id)));$columns=null;if(!is_array($rows)||empty($rows)){$columns=$wpdb->get_col("SHOW COLUMNS FROM {$table}",0);}if((!is_array($rows)||empty($rows))&&!empty($page_url)&&is_array($columns)&&in_array('submission',$columns,true)){$normalized=esc_url_raw($page_url);$candidates=array_values(array_unique(array_filter(array($normalized,rtrim($normalized,'/'),trailingslashit($normalized)))));foreach($candidates as $candidate){$like='%'.$wpdb->esc_like($candidate).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,date_time FROM {$table} WHERE submission LIKE %s ORDER BY id DESC LIMIT 5000",$like));if(is_array($rows)&&!empty($rows))break;}}if(!is_array($rows)||empty($rows)){if($avada_global_rows===null){$avada_global_rows=$wpdb->get_results("SELECT id,date_time FROM {$table} ORDER BY id DESC LIMIT 5000");}$rows=$avada_global_rows;}if(!is_array($rows)||empty($rows))return array();$result=array();foreach($rows as $r){$result[]=array('id'=>'avada_db_'.$r->id,'ts'=>$r->date_time);}return $result;default:return null;}}
