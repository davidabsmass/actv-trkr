...
public static function sync_entry_ids($discovered,$domain,$opts){$forms_with_entries=array();foreach($discovered as $form_info){$provider=$form_info['provider']??'';$form_id=$form_info['form_id']??'';$page_url=$form_info['page_url']??null;if(!$form_id)continue;$entry_ids=self::get_active_entry_ids($provider,$form_id,$page_url);if($entry_ids===null)continue;if($provider==='avada'&&!empty($entry_ids)&&is_array($entry_ids[0])){$ids=array_map(function($e){return $e['id'];},$entry_ids);$timestamps=array();foreach($entry_ids as $e){$timestamps[$e['id']]=$e['ts'];}$forms_with_entries[]=array('form_id'=>$form_id,'provider'=>$provider,'entry_ids'=>$ids,'entry_timestamps'=>$timestamps);}else{$forms_with_entries[]=array('form_id'=>$form_id,'provider'=>$provider,'entry_ids'=>$entry_ids);}}if(empty($forms_with_entries))return array('trashed'=>0,'restored'=>0);$endpoint=rtrim($opts['endpoint_url'],'/').'/sync-entries';$response=wp_remote_post($endpoint,array('timeout'=>30,'headers'=>array('Content-Type'=>'application/json','Authorization'=>'Bearer '.$opts['api_key']),'body'=>wp_json_encode(array('domain'=>$domain,'forms'=>$forms_with_entries))));if(is_wp_error($response)){error_log('[ACTV TRKR] Entry sync error: '.$response->get_error_message());return array('trashed'=>0,'restored'=>0,'error'=>$response->get_error_message());}$body=json_decode(wp_remote_retrieve_body($response),true);return array('trashed'=>$body['trashed']??0,'restored'=>$body['restored']??0);}
private static function get_active_entry_ids($provider,$form_id,$page_url=null){global $wpdb;switch($provider){case 'gravity_forms':if(!class_exists('GFAPI'))return null;$entries=\\GFAPI::get_entries($form_id,array('status'=>'active'),null,array('offset'=>0,'page_size'=>5000));if(!is_array($entries))return array();return array_map(function($e){return (string)($e['id']??'');},$entries);case 'wpforms':if(!function_exists('wpforms')||!isset(wpforms()->entry))return null;$entries=wpforms()->entry->get_entries(array('form_id'=>$form_id));if(!is_array($entries))return array();return array_map(function($e){return (string)$e->entry_id;},$entries);case 'avada':$candidate_tables=array($wpdb->prefix.'fusion_form_submissions',$wpdb->prefix.'fusionbuilder_form_submissions',$wpdb->prefix.'avada_form_submissions');$table=null;foreach($candidate_tables as $ct){if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$ct))===$ct){$table=$ct;break;}}if(!$table){error_log('[ACTV TRKR] Avada entry sync: no submission table found. Tried: '.implode(', ',$candidate_tables));return null;}$columns=$wpdb->get_col("SHOW COLUMNS FROM {$table}",0);if(!is_array($columns)||empty($columns)){error_log('[ACTV TRKR] Avada entry sync: could not read columns from '.$table);return null;}$ts_col=null;$ts_candidates=array('date_time','created_at','submitted_at','date','created');foreach($ts_candidates as $tc){if(in_array($tc,$columns,true)){$ts_col=$tc;break;}}if(!$ts_col){error_log('[ACTV TRKR] Avada entry sync: no timestamp column in '.$table.'. Columns: '.implode(', ',$columns));$ts_col='id';}$has_form_id_col=in_array('form_id',$columns,true);$has_submission_col=in_array('submission',$columns,true);static $avada_global_rows=null;$rows=array();if($has_form_id_col){$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE form_id=%d ORDER BY id DESC LIMIT 5000",intval($form_id)));}if((!is_array($rows)||empty($rows))&&!empty($page_url)&&$has_submission_col){$normalized=esc_url_raw($page_url);$url_candidates=array_values(array_unique(array_filter(array($normalized,rtrim($normalized,'/'),trailingslashit($normalized)))));foreach($url_candidates as $uc){$like='%'.$wpdb->esc_like($uc).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE submission LIKE %s ORDER BY id DESC LIMIT 5000",$like));if(is_array($rows)&&!empty($rows))break;}}if(!is_array($rows)||empty($rows)){if($avada_global_rows===null){$avada_global_rows=$wpdb->get_results("SELECT id,{$ts_col} AS ts FROM {$table} ORDER BY id DESC LIMIT 5000");}$rows=$avada_global_rows;}if(!is_array($rows)||empty($rows)){error_log('[ACTV TRKR] Avada entry sync: form_id='.$form_id.' table='.$table.' — 0 rows');return array();}error_log('[ACTV TRKR] Avada entry sync: form_id='.$form_id.' table='.$table.' — found '.count($rows).' entries');$result=array();foreach($rows as $r){$result[]=array('id'=>'avada_db_'.$r->id,'ts'=>$r->ts);}return $result;default:return null;}}
...
private static function get_avada_db_entry_id($form_post_id,$avada_data=array()){global $wpdb;$table=$wpdb->prefix.'fusion_form_submissions';if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$table))!==$table)return 'avada_'.time().'_'.wp_rand();if(is_array($avada_data)&&!empty($avada_data['submission'])&&is_string($avada_data['submission'])){$parts=array_map('trim',explode(',',$avada_data['submission']));$submitted_at=$parts[1]??'';if($submitted_at){$row=$wpdb->get_row($wpdb->prepare("SELECT id FROM {$table} WHERE date_time=%s ORDER BY id DESC LIMIT 1",$submitted_at));if($row&&isset($row->id))return 'avada_db_'.$row->id;}}$row=$wpdb->get_row($wpdb->prepare("SELECT id FROM {$table} WHERE form_id=%d ORDER BY id DESC LIMIT 1",intval($form_post_id)));if($row&&isset($row->id))return 'avada_db_'.$row->id;return 'avada_'.time().'_'.wp_rand();}
