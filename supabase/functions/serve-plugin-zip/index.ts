...
private static function get_active_entry_ids($provider,$form_id,$page_url=null){global $wpdb;switch($provider){case 'gravity_forms':if(!class_exists('GFAPI'))return null;$entries=\\GFAPI::get_entries($form_id,array('status'=>'active'),null,array('offset'=>0,'page_size'=>5000));if(!is_array($entries))return array();return array_map(function($e){return (string)($e['id']??'');},$entries);case 'wpforms':if(!function_exists('wpforms')||!isset(wpforms()->entry))return null;$entries=wpforms()->entry->get_entries(array('form_id'=>$form_id));if(!is_array($entries))return array();return array_map(function($e){return (string)$e->entry_id;},$entries);case 'avada':$candidate_tables=array($wpdb->prefix.'fusion_form_submissions',$wpdb->prefix.'fusionbuilder_form_submissions',$wpdb->prefix.'avada_form_submissions');$table=null;foreach($candidate_tables as $ct){if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$ct))===$ct){$table=$ct;break;}}if(!$table){error_log('[ACTV TRKR] Avada: no table found');self::$last_avada_strategy='no_table';return null;}$columns=$wpdb->get_col("SHOW COLUMNS FROM {$table}",0);if(!is_array($columns)||empty($columns)){self::$last_avada_strategy='no_columns';return null;}$ts_col=null;$ts_cands=array('date_time','created_at','submitted_at','date','created','updated_at');foreach($ts_cands as $tc){if(in_array($tc,$columns,true)){$ts_col=$tc;break;}}if(!$ts_col)$ts_col='id';$form_ref_cands=array('form_id','fusion_form_id','post_id','parent_id','form_post_id');$blob_cands=array('submission','data','fields','form_data','meta','content');$rows=array();$strategy='none';foreach($form_ref_cands as $frc){if(!in_array($frc,$columns,true))continue;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE {$frc}=%d ORDER BY id DESC LIMIT 5000",intval($form_id)));if(is_array($rows)&&!empty($rows)){$strategy='form_ref:'.$frc;break;}$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE {$frc}=%s ORDER BY id DESC LIMIT 5000",(string)$form_id));if(is_array($rows)&&!empty($rows)){$strategy='form_ref_str:'.$frc;break;}}if(!is_array($rows)||empty($rows)){foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;$like='%'.$wpdb->esc_like('"form_id":"'.$form_id.'"').'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE {$bc} LIKE %s ORDER BY id DESC LIMIT 5000",$like));if(is_array($rows)&&!empty($rows)){$strategy='blob_fid:'.$bc;break;}$like2='%form_id%'.$wpdb->esc_like($form_id).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE {$bc} LIKE %s ORDER BY id DESC LIMIT 5000",$like2));if(is_array($rows)&&!empty($rows)){$strategy='blob_fid_loose:'.$bc;break;}}}if((!is_array($rows)||empty($rows))&&!empty($page_url)){$norm=esc_url_raw($page_url);$urls=array_values(array_unique(array_filter(array($norm,rtrim($norm,'/'),trailingslashit($norm)))));foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;foreach($urls as $uc){$like='%'.$wpdb->esc_like($uc).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,{$ts_col} AS ts FROM {$table} WHERE {$bc} LIKE %s ORDER BY id DESC LIMIT 5000",$like));if(is_array($rows)&&!empty($rows)){$strategy='blob_url:'.$bc;break 2;}}}}self::$last_avada_strategy=$strategy;if(!is_array($rows)||empty($rows)){error_log('[ACTV TRKR] Avada: form_id='.$form_id.' 0 rows (strategy='.$strategy.')');return array();}error_log('[ACTV TRKR] Avada: form_id='.$form_id.' '.count($rows).' entries (strategy='.$strategy.')');$result=array();foreach($rows as $r){$result[]=array('id'=>'avada_db_'.$r->id,'ts'=>$r->ts);}return $result;default:return null;}}
