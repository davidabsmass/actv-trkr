...
public static function register_rest_routes(){register_rest_route('actv-trkr/v1','/sync',array('methods'=>'POST','callback'=>array(__CLASS__,'handle_rest_sync'),'permission_callback'=>'__return_true'));register_rest_route('actv-trkr/v1','/backfill-avada',array('methods'=>'POST','callback'=>array(__CLASS__,'handle_rest_backfill_avada'),'permission_callback'=>'__return_true'));}
...
private static function get_active_entry_ids($provider,$form_id,$page_url=null,$form_title=null){global $wpdb;switch($provider){case 'gravity_forms':if(!class_exists('GFAPI'))return null;$entries=\GFAPI::get_entries($form_id,array('status'=>'active'),null,array('offset'=>0,'page_size'=>5000));if(!is_array($entries))return array();return array_map(function($e){return (string)($e['id']??'');},$entries);case 'wpforms':if(!function_exists('wpforms')||!isset(wpforms()->entry))return null;$entries=wpforms()->entry->get_entries(array('form_id'=>$form_id));if(!is_array($entries))return array();return array_map(function($e){return (string)$e->entry_id;},$entries);case 'avada':$candidate_tables=array($wpdb->prefix.'fusion_form_submissions',$wpdb->prefix.'fusionbuilder_form_submissions',$wpdb->prefix.'avada_form_submissions',$wpdb->prefix.'fusion_form_entries',$wpdb->prefix.'avada_submissions',$wpdb->prefix.'fusion_submissions');$table=null;foreach($candidate_tables as $ct){if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$ct))===$ct){$table=$ct;break;}}if(!$table){$wildcard_results=$wpdb->get_col("SHOW TABLES LIKE '%fusion%form%'");if(empty($wildcard_results)){$wildcard_results=$wpdb->get_col("SHOW TABLES LIKE '%avada%'");}if(!empty($wildcard_results)){foreach($wildcard_results as $wt){$lw=strtolower($wt);if(strpos($lw,'submission')!==false||strpos($lw,'entry')!==false||strpos($lw,'entries')!==false){$table=$wt;break;}}if(!$table)$table=$wildcard_results[0];}if(!$table){error_log('[ACTV TRKR] Avada: no table found. Searched: '.implode(',',$candidate_tables));self::$last_avada_strategy='no_table';return null;}}error_log('[ACTV TRKR] Avada: using table='.$table.' for form_id='.$form_id);$columns=$wpdb->get_col("SHOW COLUMNS FROM ".$table,0);if(!is_array($columns)||empty($columns)){self::$last_avada_strategy='no_columns';return null;}error_log('[ACTV TRKR] Avada: columns='.implode(',',$columns));$ts_col=null;$ts_cands=array('date_time','created_at','submitted_at','date','created','updated_at','timestamp','submit_date','submission_date','date_submitted','submitted_on');foreach($ts_cands as $tc){if(in_array($tc,$columns,true)){$ts_col=$tc;break;}}if(!$ts_col)$ts_col='id';$total_rows=$wpdb->get_var("SELECT COUNT(*) FROM ".$table);error_log('[ACTV TRKR] Avada: table total_rows='.$total_rows);$form_ref_cands=array('form_id','fusion_form_id','post_id','parent_id','form_post_id','form_ref','source_id','formid','source_form_id','fusion_form');$blob_cands=array('submission','data','fields','form_data','meta','content','submission_data','form_fields','response','payload','entry_data','source_url','page_url','post_url','request_uri','referer','http_referer');$rows=array();$strategy='none';foreach($form_ref_cands as $frc){if(!in_array($frc,$columns,true))continue;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$frc."=%d ORDER BY id DESC LIMIT 5000",intval($form_id)));if(is_array($rows)&&!empty($rows)){$strategy='form_ref:'.$frc;break;}$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$frc."=%s ORDER BY id DESC LIMIT 5000",(string)$form_id));if(is_array($rows)&&!empty($rows)){$strategy='form_ref_str:'.$frc;break;}}if(!is_array($rows)||empty($rows)){foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;$like='%'.$wpdb->esc_like('"form_id":"'.$form_id.'"').'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$bc." LIKE %s ORDER BY id DESC LIMIT 5000",$like));if(is_array($rows)&&!empty($rows)){$strategy='blob_fid:'.$bc;break;}$like2='%'.$wpdb->esc_like('"'.$form_id.'"').'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$bc." LIKE %s ORDER BY id DESC LIMIT 5000",$like2));if(is_array($rows)&&!empty($rows)){$strategy='blob_fid_quoted:'.$bc;break;}$like3='%form_id%'.$wpdb->esc_like($form_id).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$bc." LIKE %s ORDER BY id DESC LIMIT 5000",$like3));if(is_array($rows)&&!empty($rows)){$strategy='blob_fid_loose:'.$bc;break;}}}if(!is_array($rows)||empty($rows)){foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;$marker_likes=array('%'.$wpdb->esc_like('\"fusion_form_id\":\"'.(string)$form_id.'\"').'%','%'.$wpdb->esc_like('\"fusion_form_id\":'.intval($form_id)).'%','%'.$wpdb->esc_like('\"form_post_id\":\"'.(string)$form_id.'\"').'%','%'.$wpdb->esc_like('\"form_post_id\":'.intval($form_id)).'%','%'.$wpdb->esc_like('\"post_id\":\"'.(string)$form_id.'\"').'%','%'.$wpdb->esc_like('\"post_id\":'.intval($form_id)).'%','%'.$wpdb->esc_like('s:14:\"fusion_form_id\";i:'.intval($form_id)).'%','%'.$wpdb->esc_like('s:12:\"form_post_id\";i:'.intval($form_id)).'%','%'.$wpdb->esc_like('s:7:\"form_id\";i:'.intval($form_id)).'%');foreach($marker_likes as $m_like){$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$bc." LIKE %s ORDER BY id DESC LIMIT 5000",$m_like));if(is_array($rows)&&!empty($rows)){$strategy='blob_marker:'.$bc;break 2;}}}}if(!is_array($rows)||empty($rows))&&!empty($page_url)){$norm=esc_url_raw($page_url);$urls=array_values(array_unique(array_filter(array($norm,rtrim($norm,'/'),trailingslashit($norm)))));foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;foreach($urls as $uc){$like='%'.$wpdb->esc_like($uc).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$bc." LIKE %s ORDER BY id DESC LIMIT 5000",$like));if(is_array($rows)&&!empty($rows)){$strategy='blob_url:'.$bc;break 2;}}}}if(!is_array($rows)||empty($rows))){$resolved_title=is_string($form_title)&&$form_title!==''?$form_title:get_the_title(intval($form_id));if(is_string($resolved_title)){$resolved_title=trim($resolved_title);}if(!empty($resolved_title)){$name_cands=array('form_name','name','title','form_title');foreach($name_cands as $nc){if(!in_array($nc,$columns,true))continue;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$nc."=%s ORDER BY id DESC LIMIT 5000",$resolved_title));if(is_array($rows)&&!empty($rows)){$strategy='name_exact:'.$nc;break;}$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$nc." LIKE %s ORDER BY id DESC LIMIT 5000",'%'.$wpdb->esc_like($resolved_title).'%'));if(is_array($rows)&&!empty($rows)){$strategy='name_like:'.$nc;break;}}if(!is_array($rows)||empty($rows))){foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE ".$bc." LIKE %s ORDER BY id DESC LIMIT 5000",'%'.$wpdb->esc_like($resolved_title).'%'));if(is_array($rows)&&!empty($rows)){$strategy='blob_title:'.$bc;break;}}}}}if(!is_array($rows)||empty($rows))&&!empty($resolved_title)){$normalized_title=trim(preg_replace('/[^a-z0-9]+/i',' ',strtolower($resolved_title)));if($normalized_title!==''){foreach($blob_cands as $bc){if(!in_array($bc,$columns,true))continue;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,".$ts_col." AS ts FROM ".$table." WHERE LOWER(REPLACE(REPLACE(".$bc.",'-',' '),'_',' ')) LIKE %s ORDER BY id DESC LIMIT 5000",'%'.$wpdb->esc_like($normalized_title).'%'));if(is_array($rows)&&!empty($rows)){$strategy='blob_title_normalized:'.$bc;break;}}}}if(!is_array($rows)||empty($rows))){$all_rows=$wpdb->get_results("SELECT * FROM ".$table." ORDER BY id DESC LIMIT 5000",ARRAY_A);$filtered=array();if(is_array($all_rows)&&!empty($all_rows)){$needles=array('"form_id":"'.(string)$form_id.'"','"form_id":'.intval($form_id),'"fusion_form_id":"'.(string)$form_id.'"','"fusion_form_id":'.intval($form_id),'"form_post_id":"'.(string)$form_id.'"','"form_post_id":'.intval($form_id),'"post_id":"'.(string)$form_id.'"','"post_id":'.intval($form_id),'s:7:"form_id";i:'.intval($form_id),'s:14:"fusion_form_id";i:'.intval($form_id),'s:12:"form_post_id";i:'.intval($form_id));foreach($all_rows as $ar){$match=false;foreach($form_ref_cands as $frc){if(isset($ar[$frc])&&(string)$ar[$frc]===(string)$form_id){$match=true;break;}}if(!$match){$json=wp_json_encode($ar);foreach($needles as $needle){if($needle!==''&&strpos($json,$needle)!==false){$match=true;break;}}}if($match){$filtered[]=(object)array('id'=>$ar['id'],'ts'=>($ar[$ts_col]??null));}}}if(!empty($filtered)){$rows=$filtered;$strategy='small_table_filtered';}}self::$last_avada_strategy=$strategy;if(!is_array($rows)||empty($rows)){error_log('[ACTV TRKR] Avada: form_id='.$form_id.' 0 rows (strategy='.$strategy.' table='.$table.' cols='.implode(',',$columns).' total='.$total_rows.')');return array();}error_log('[ACTV TRKR] Avada: form_id='.$form_id.' '.count($rows).' entries (strategy='.$strategy.')');$result=array();foreach($rows as $r){$result[]=array('id'=>'avada_db_'.$r->id,'ts'=>$r->ts);}return $result;default:return null;}}
