...
private static function get_active_entry_ids($provider,$form_id,$page_url=null){global $wpdb;switch($provider){case 'gravity_forms':if(!class_exists('GFAPI'))return null;$entries=\\GFAPI::get_entries($form_id,array('status'=>'active'),null,array('offset'=>0,'page_size'=>5000));if(!is_array($entries))return array();return array_map(function($e){return (string)($e['id']??'');},$entries);case 'wpforms':if(!function_exists('wpforms')||!isset(wpforms()->entry))return null;$entries=wpforms()->entry->get_entries(array('form_id'=>$form_id));if(!is_array($entries))return array();return array_map(function($e){return (string)$e->entry_id;},$entries);case 'avada':$table=$wpdb->prefix.'fusion_form_submissions';if($wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s",$table))!==$table)return null;$rows=$wpdb->get_results($wpdb->prepare("SELECT id,date_time FROM {$table} WHERE form_id=%d AND is_read>=0 ORDER BY id DESC LIMIT 5000",intval($form_id)));if((!is_array($rows)||empty($rows))&&!empty($page_url)){$columns=$wpdb->get_col("SHOW COLUMNS FROM {$table}",0);if(is_array($columns)&&in_array('submission',$columns,true)){$like='%'.$wpdb->esc_like(esc_url_raw($page_url)).'%';$rows=$wpdb->get_results($wpdb->prepare("SELECT id,date_time FROM {$table} WHERE submission LIKE %s AND is_read>=0 ORDER BY id DESC LIMIT 5000",$like));}}if(!is_array($rows)||empty($rows))return array();$result=array();foreach($rows as $r){$result[]=array('id'=>'avada_db_'.$r->id,'ts'=>$r->date_time);}return $result;default:return null;}}
