...
BEGIN
  IF p_allow_existing THEN
    -- Idempotency guard for onboarding: if the caller already belongs to an
    -- org, reuse it instead of creating a duplicate. Prevents retries (slow
    -- checkout, double-clicks, browser back/forward) from creating empty orgs.
