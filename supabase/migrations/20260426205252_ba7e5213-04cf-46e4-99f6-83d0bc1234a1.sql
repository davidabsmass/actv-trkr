CREATE OR REPLACE FUNCTION public.get_session_journey_stats(p_org_id uuid, p_start timestamp with time zone, p_end timestamp with time zone, p_site_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  all_leads_in_range AS (
    SELECT COUNT(*)::int AS n
    FROM public.leads l
    WHERE l.org_id = p_org_id
      AND COALESCE(l.status, '') <> 'trashed'
  ...
  SELECT jsonb_build_object(
    ...
    'total_leads', (SELECT n FROM all_leads_in_range),
